...
const ChatWindow = ({ exchangeId, exchangeTitle, onClose }) => {
  ...
  return (
    <div className="fixed bottom-4 right-4 w-96 h-[500px] bg-transparent rounded-xl border border-gray-200 flex flex-col z-50">
      {/* Header */}
      <div className="bg-white text-black font-bold p-4 rounded-t-xl flex justify-between items-center ">
        <div>
          ...
          return (
            <div key={idx} className={`flex ${isMe ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[80%] rounded-2xl px-4 py-2 text-sm ${isMe
                  ...
                  : 'bg-transparent text-white border border-gray-200 rounded-bl-none'
                }`}>
                <p>{msg.text}</p>
      ...
      {/* Input Area */}
      <form onSubmit={handleSendMessage} className="p-3 bg-transparent border-t border-[#333333] rounded-b-xl flex items-center gap-2">
        <input
          type="text"
          value={newMessage}
        ...
        <button
          type="submit"
          className="btn btn-sm btn-circle btn-primary bg-white text-black font-bold hover:bg-gray-200 border-none text-white transform active:scale-95 transition-all"
